...
class TelemetrySampler:
    ...
    def assert_safe(self) -> None:
        """Abort between request cases if telemetry crossed a safety gate."""
        if self.violation:
        ...
        with self.output.open("w", encoding="utf-8") as handle:
            while not self.stop_event.is_set():
                try:
                    ...
                    if record["core_temp_c"] >= 80:
                        self.violation = f"core temperature reached {record['core_temp_c']} C"
                    if record["memory_temp_c"] >= 85:
                        self.violation = (
                            f"memory temperature reached {record['memory_temp_c']} C"
                        )
                    handle.write(json.dumps(record, sort_keys=True) + "\n")
                    handle.flush()
                except Exception as error:  # Abort cleanly between request cases.
                    self.violation = f"GPU telemetry failed: {error}"
