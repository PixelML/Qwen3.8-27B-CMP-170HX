...
GPU_INDEX=${1:?usage: run-chimera-card.sh GPU_INDEX OUTPUT_DIR}
OUTPUT_DIR=${2:?usage: run-chimera-card.sh GPU_INDEX OUTPUT_DIR}
PORT=$((18020 + GPU_INDEX))
API_KEY=qwen38-chimera-bench
QWEN_REPO=/home/ubuntu/repos/qwen-serving
MODEL=/library/models/qwen38/bench-2026-08-29/Qwen3.8-27B-W4A16-AutoRound-fast
...

{
  ...
  /models/venvs/qwen38/bin/pip show vllm torch flashinfer-python flashinfer-cubin \
    | grep -E '^(Name|Version):'
...
export PATH=/models/venvs/qwen38/bin:$PATH
export HF_HOME=/library/models/hf-cache
...
sudo -n journalctl -k --since "$STARTED_AT_JOURNAL" --no-pager \
