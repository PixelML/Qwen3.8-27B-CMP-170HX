#!/usr/bin/env bash
# v9: fix the v8 decode gap. Three changes vs v8:
#  1. /app/prepare symlink so docker/prepare.sh (which cd's to /app) actually
#     finds its scripts -> int8 lm_head/embed/MTP quant + the +15% fast variant
#     get built instead of prepare.sh dying on quant_lm_head.py.
#  2. curand.h made visible to the flashinfer JIT so the fast-topk kernel
#     builds (v8 silently fell back to torch.topk).
#  3. Benchmark runs ON the instance after health, so stdout captures decode,
#     prefill, and peak GPU stats without needing SSH.
set -ux
date -u
...
# curand.h for the flashinfer fast-topk JIT (v8: fatal error curand.h -> torch.topk fallback)
...
cd /app/qwen-serving
SPEC=dflash2 CTX=fast MAX_SEQS=1 DFLASH_TOKENS=7 PORT=18020 GPU_UTIL=0.90 KV_MEM= \
...
[ "$HEALTH" = "1" ] || { echo "HEALTH-NEVER-OK"; tail -n 60 /tmp/qwen38-server.log; exit 0; }

sleep 20
venv/bin/python - <<'PYEOF'
import json, subprocess, threading, time, urllib.request
BASE_URL = "http://127.0.0.1:18020"
KEY = "qwen38-bench-20260828"
...
def bench(name, prompt, maxtok, warm, samples, ptok):
    ...
    for i in range(samples):
        ...
        print(json.dumps({"run": name, "i": i, "ttft_ms": round(ttft * 1000, 1), "total_s": round(total, 3),
    ...
    print(json.dumps({"summary": name, "prompt_tokens": ptok, "mean_ttft_ms": round(ttft, 1),
...
print(json.dumps({"prompt_tokens": {"story": t256, "long": tlong}}), flush=True)
...
print(json.dumps({"gpu_peak": peak}), flush=True)
PYEOF
echo "BENCH-DONE"
grep -i -E "accept|spec" /tmp/qwen38-server.log | tail -n 20 || true
tail -n 30 /tmp/qwen38-server.log || true
